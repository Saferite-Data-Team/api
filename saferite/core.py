...
class API:
    ...
    def _standard_call(self, module: str, call_type: str, data: dict = None, json_data=None,** kwargs):
        ...
        return getattr(requests, call_type)(url=self.payload, headers=self.headers, params=self.params, data=data, json=json_data).json()
...
class Data:
    ...
    def __post_init__(self):
        for field in fields(self):
            ...
            if not isinstance(value, field.type) and value is not None:
                raise ValueError(
                    f'Expected {field.name} to be {field.type}, 'f'got {type(value)}')
    ...
    def from_dataframe(cls, df, field_name: str, field_value: str, repeated: bool = False, **kwargs):
        ...
        class_columns = set(
            cls.__match_args__) if cls.__match_args__ != () else set(cls.fields())
        df_columns = set(df.columns)
        columns = list(df_columns.intersection(class_columns))

        data = df.loc[df[field_name] ==
                      field_value, columns].to_dict('records')
        ...
        for key in all_args:
            if key in hints:
                if type(all_args[key]) != hints[key]:
                    raise Exception(
                        f'Type of {key} is {type(all_args[key])} and not {hints[key]}')
